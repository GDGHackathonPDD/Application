...
2. **Env:** `.env.local` at this `Application/` root holds Clerk, Convex, **`AGENT_API_*`**, and **`OPENROUTER_API_KEY`**. The Python Agent API lives in **`../Agent/`** at the repo root and **reads `Application/.env.local` automatically** when present (see `../Agent/README.md`). You can also use `Agent/.env`. In the **Convex dashboard**, set `AGENT_API_URL` (e.g. `http://127.0.0.1:8000` locally, or your stable `https://agent.*.sslip.io` / Cloud Run URL in production) and `AGENT_API_KEY` so actions can call `/decompose`, `/plan-copy`, and `/daily-summary`.
